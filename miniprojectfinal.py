...
def add_product_price(price):
    
    while True:
        try:
            print("\nEnter price of new product:\n")
            new_product_price = float(price())
...
def add_new_product_db():
    ...
    price_input = add_product_price(input)
...
def id_update_product_func(testing):
    append_product_table_to_list
    while True:
...
def showing_current_product_name(id_input):
    for product in products:
        if product["Product ID"] == id_input:
...
def update_product_db():
    ...
    id_input = id_update_product_func(input)
...
# Uncomment following code once unit test step is complete
append_product_table_to_list()
append_courier_table_to_list()
append_order_table_to_list()
main_menu()
